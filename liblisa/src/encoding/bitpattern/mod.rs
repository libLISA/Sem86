...
/// A packed version of [`Bit`], using only one byte per bit rather than two.
///
/// `0x00..0xfc` are used to store part indexes for `Bit::Part`,
/// `0xfd` is used for `Bit::DontCare`,
/// `0xfe` is used for `Bit::Fixed(0)` and
/// `0xff` is use| for `Bit::Fixed(1)`.
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
#[cfg_attr(feature = "mem_dbg", derive(mem_dbg::MemSize))]
#[derive(Clone, Copy, PartialEq, Eq, Hash, Encode, Decode)]
pub struct PackedBit(u8);

impl Debug for PackedBit {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        Bit::from(*self).fmt(f)
    }
}

// Delegate `Serialize`/`Deserialize` to the `Bit` implementation for backwards-compatibility.
impl Serialize for PackedBit {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        Bit::from(*self).serialize(serializer)
    }
}

impl<'de> Deserialize<'de> for PackedBit {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        Ok(Bit::deserialize(deserializer)?.into())
    }
}

impl From<PackedBit> for Bit {
    fn from(value: PackedBit) -> Self {
        if value.0 < 0xfd {
            Bit::Part(value.0)
        } else if value.0 == 0xfd {
            Bit::DontCare
        } else {
            Bit::Fixed(value.0 & 1)
        }
...
impl From<Bit> for PackedBit {
    fn from(value: Bit) -> Self {
        match value {
            Bit::Part(part_index) => {
                assert!(part_index < 0xfd);
                Self(part_index)
            },
            Bit::Fixed(val) => {
                assert!(val == 0 || val == 1);
...
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
#[cfg_attr(feature = "mem_dbg", derive(mem_dbg::MemSize))]
#[derive(Debug, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, Encode, Decode, Hash)]
...
impl ImmBitOrder {
    ...
    pub fn as_offset(&self, source_bit: u64) -> u64 {
        match self {
            ImmBitOrder::Positive(bit) => source_bit.wrapping_shl(*bit as u32),
            ImmBitOrder::Negative(bit) => (!(source_bit.wrapping_shl(*bit as u32))).wrapping_add(1),
        }
    }
}
...
impl MappingOrBitOrder {
    /// Computes the effective value of the immediate.
    pub fn compute(&self, part_val: u64) -> Option<u64> {
        match self {
            Self::Mapping(vals) => {
                if vals[part_val as usize].is_valid() {
                    Some(part_val)
                } else {
                    None
                }
            },
            Self::BitOrder(bit_order) => Some(bit_order.iter().enumerate().fold(0, |acc: u64, (index, bit)| {
                acc.wrapping_add(bit.as_offset((part_val >> index) & 1))
            })),
        }
    }
}
