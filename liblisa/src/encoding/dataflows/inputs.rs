...
#[derive(Clone, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize, Encode, Decode, Default)]
#[serde(bound = "")]
...
impl<A: Arch> Inputs<A> {
    ...
    #[inline]
    pub fn unsorted(inputs: Vec<ParLoc<A>>) -> Self {
        Inputs {
            inputs,
        }
    }
    ...
    /// For example, if the inputs are part of a dataflow in an encoding.
    #[inline]
    pub fn sorted(mut inputs: Vec<ParLoc<A>>) -> Self {
        inputs.sort();
    ...
    #[inline]
    pub fn iter(&self) -> impl Iterator<Item = &ParLoc<A>> {
        self.inputs.iter()
    }

    /// Iterates over `&mut` references to all inputs.
    #[inline]
    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut ParLoc<A>> {
        self.inputs.iter_mut()
    }

    /// Removes all inputs for which `f` returns false.
    #[inline]
    pub fn retain(&mut self, f: impl FnMut(&ParLoc<A>) -> bool) {
    ...
    #[inline]
    pub fn contains(&self, item: &ParLoc<A>) -> bool {
    ...
    #[inline]
    pub fn remove(&mut self, item: &ParLoc<A>) {
    ...
    #[inline]
    pub fn push(&mut self, item: ParLoc<A>) {
        self.inputs.push(item)
    }
}

impl<A: Arch> Index<usize> for Inputs<A> {
    type Output = ParLoc<A>;

    #[inline]
    fn index(&self, index: usize) -> &ParLoc<A> {
        &self.inputs[index]
    }
}
...
impl<A: Arch> IndexMut<usize> for Inputs<A> {
    #[inline]
    fn index_mut(&mut self, index: usize) -> &mut ParLoc<A> {
