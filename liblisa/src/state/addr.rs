...
impl Addr {
    ...
    pub fn page<A: Arch>(self) -> Page<A> {
        Page(self.apply_size(A::ADDR_SIZE).0 >> A::PAGE_BITS, PhantomData)
    }
    ...
    pub fn apply_size(&self, addr_size: AddrSize) -> Addr {
        Addr::new(addr_size.apply(self.as_u64()))
    }
}
...
impl<A: Arch> Page<A> {
    ...
    pub fn start_addr(&self) -> Addr {
        Addr(self.0 << A::PAGE_BITS).apply_size(A::ADDR_SIZE)
    }
    ...
    pub fn last_address_of_page(&self) -> Addr {
        Addr((self.0 << A::PAGE_BITS) | ((1 << A::PAGE_BITS) - 1)).apply_size(A::ADDR_SIZE)
    }
    ...
    pub fn first_address_after_page(&self) -> Addr {
        Addr(A::ADDR_SIZE.apply((self.0 + 1) << A::PAGE_BITS)).apply_size(A::ADDR_SIZE)
    }
    ...
    pub fn contains(&self, expected_addr: Addr) -> bool {
        expected_addr.page::<A>() == *self
    }
}
...
impl<A: Arch> Add<u64> for Page<A> {
    ...
    fn add(self, rhs: u64) -> Self::Output {
        Page(
            A::ADDR_SIZE.apply(self.0.wrapping_add(rhs) << A::PAGE_BITS) >> A::PAGE_BITS,
            PhantomData,
        )
    }
}
...
impl<A: Arch> Sub<u64> for Page<A> {
    ...
    fn sub(self, rhs: u64) -> Self::Output {
        Page(
            A::ADDR_SIZE.apply(self.0.wrapping_sub(rhs) << A::PAGE_BITS) >> A::PAGE_BITS,
            PhantomData,
        )
    }
}
...
impl Area {
    ...
    #[inline]
    pub fn contains<A: Arch>(&self, addr: Addr) -> bool {
        if let Some(end_addr) = self.addr.0.checked_add(self.size).and_then(|x| A::ADDR_SIZE.try_apply(x)) {
            ...
            self.offset(offset).contains::<A>(addr + offset)
        }
    }
    ...
    #[inline]
    pub fn end_addr<A: Arch>(&self) -> Addr {
        Addr::new(A::ADDR_SIZE.apply(self.addr.0.wrapping_add(self.size - 1)))
    }
    ...
    #[inline]
    pub fn crosses_page_bounds<A: Arch>(&self) -> bool {
        let start_index = self.start_addr().0 >> A::PAGE_BITS;
        let end_index = self.end_addr::<A>().as_u64() >> A::PAGE_BITS;
    ...
    pub fn shares_page_with<A: Arch>(&self, other: &Area) -> bool {
        self.expand_to_page::<A>().overlaps_with::<A>(&other.expand_to_page::<A>())
    }

    /// Returns the last address if it doesn't wrap
    #[inline]
    fn last_addr<A: Arch>(&self) -> Option<Addr> {
        self.addr
            .0
            .checked_add(self.size - 1)
            .and_then(|x| Some(Addr(A::ADDR_SIZE.try_apply(x)?)))
    }

    /// Returns true if the area overlaps with `other`.
    #[inline]
    pub fn overlaps_with<A: Arch>(&self, other: &Area) -> bool {
        // println!("Checking overlap between {:?} and {:?}", self, other);
        match (self.last_addr::<A>(), other.last_addr::<A>()) {
            (Some(end), Some(other_end)) => other_end >= self.addr && other.addr <= end,
            (Some(_), None) | (None, Some(_)) => {
                let offset_self_to_0 = 0u64.wrapping_sub(self.addr.as_u64());
                let offset = if other.offset(offset_self_to_0).last_addr::<A>().is_some() {
                    offset_self_to_0
                } else {
                ...
                let end = new_self.last_addr::<A>().expect("Areas are too far apart");
                let other_end = new_other.last_addr::<A>().expect("Areas are too far apart");
