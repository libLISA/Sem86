...
pub struct SystemState<A: Arch> {
    ...
    pub randomize_unused_memory: bool,
...
impl<A: Arch> SystemState<A> {
    ...
    #[inline]
    pub fn with_location(&self, loc: &UnsizedLoc<A>, value: Value) -> Self {
    ...
    #[inline]
    pub fn get_dest(&self, dest: &ParLoc<A>) -> Value<'_> {
        let loc = UnsizedLoc::try_from(dest.loc).unwrap();
        let value = self.location(&loc);
        let size = dest.size();

        match value {
            Value::Num(_) => assert!(size.end_byte() <= 7, "{dest:?} out of range for value {value:?}"),
            Value::Bytes(b) => assert!(size.end_byte() < b.len(), "{dest:?} out of range for value {value:?}"),
        }
    ...
    pub fn set_dest(&mut self, dest: &ParLoc<A>, new_value: &Value<'_>) {
        let loc = UnsizedLoc::try_from(dest.loc).unwrap();
        let new_bytes;
        let size = dest.size();
    ...
    pub fn modify_dest(&mut self, dest: &ParLoc<A>, modify: impl FnOnce(MutValue<'_>)) {
        match dest {
            ParLoc {
                ...
                size,
            } => self.cpu_mut().modify_reg(*reg, |value| match value {
            ...
            ParLoc {
                loc: UnsizedParLoc::Const(_) | UnsizedParLoc::InstrLen | UnsizedParLoc::Part(_),
                ..
            } => panic!("unable to modify: {dest:?}"),
        }
    }
    ...
    #[inline]
    pub fn with_dest(&self, dest: &ParLoc<A>, v: &Value<'_>) -> Self {
        let mut result = self.clone();
    ...
    pub fn set_location(&mut self, loc: &UnsizedLoc<A>, value: Value) {
        match (loc, value) {
            (UnsizedLoc::Reg(reg), value) => self.set_reg(*reg, value),
            (UnsizedLoc::Memory(index), Value::Bytes(v)) => {
    ...
    #[inline]
    pub fn location(&self, loc: &UnsizedLoc<A>) -> Value<'_> {
        match loc {
            UnsizedLoc::Reg(reg) => self.cpu().reg(*reg),
            UnsizedLoc::Memory(n) => Value::Bytes(&self.memory().get(*n).2),
        }
    }

    /// Returns the value of `loc`, or returns `None` if `loc` refers to a non-existant memory access.
    #[inline]
    pub fn get_location(&self, loc: &UnsizedLoc<A>) -> Option<Value<'_>> {
        match loc {
            UnsizedLoc::Reg(reg) => Some(self.cpu().reg(*reg)),
            UnsizedLoc::Memory(n) => self.memory().get_checked(*n).as_ref().map(|v| Value::Bytes(&v.2)),
        }
    }
}
