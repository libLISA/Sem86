...
impl GrowingBitmap {
    ...
    pub fn from_slice(slice: &impl BitmapSlice) -> Self {
        ...
        debug_assert_eq!(data.len(), slice.len().div_ceil(64));
    ...
    pub fn get(&self, n: usize) -> bool {
        ...
        (self.data.get(index).copied().unwrap_or(0) >> offset) & 1 != 0
    }
    ...
    pub fn or_with(&mut self, rhs: &impl BitmapSlice) -> bool {
        ...
        self.internal_resize(new_len - 1, Self::index(new_len - 1).0);
        for (a, b) in self.data.iter_mut().zip(rhs.iter_data().chain(repeat(0))) {
            let new = *a | b;
            changed |= *a != new;
            *a = new;
    ...
    pub fn is_all_ones(&self) -> bool {
        let (full_u64s, ok) = if self.len.is_multiple_of(64) {
    ...
    pub fn remove_front(&mut self, num: usize) {
        ...
        let data_len_needed = self.len.div_ceil(64);
    ...
    pub fn remove_back(&mut self, num: usize) {
        ...
        self.data.resize(self.len.div_ceil(64), 0);
        self.zeroize_past_end();
    }
    ...
    pub fn resize(&mut self, len: usize) {
        ...
        let required_len = len.div_ceil(64);
...
mod tests {
    ...
    pub fn is_superset_of() {
        ...
        let b3 = [false, false, true, true]
            .into_iter()
            .chain(std::iter::repeat_n(false, 500))
            .collect::<GrowingBitmap>();
        let b4 = [false, false, true, true]
            .into_iter()
            .chain(std::iter::repeat_n(false, 350))
            .chain(once(true))
            .collect::<GrowingBitmap>();
