...
pub trait AsValue: Debug + Hash + PartialEq<OwnedValue> {
    ...
    fn unwrap_num(&self) -> u64 {
        match self.as_value() {
            Value::Num(v) => v,
            _ => panic!("called unwrap_num on Value::Bytes"),
        }
    }
...
impl MutValue<'_> {
    ...
    /// If the value is a [`MutValue::Num`], masks (ANDs) the value in-place.
    /// Does nothing if the value is a [`MutValue::Bytes`].
    pub fn mask(&mut self, mask: u64) {
        match self {
            MutValue::Num(n) => **n &= mask,
            MutValue::Bytes(_) => (),
        }
    }
}
...
impl Value<'_> {
    ...
    pub fn slice(&self, size: Size) -> Self {
        let w = size.num_bytes() * 8;
        match self {
            Value::Num(n) => Value::Num((n >> (size.start_byte() * 8)) & bitmask_u64(w as u32)),
            Value::Bytes(b) => {
    ...
    /// If the value is a [`Value::Num`], it is masked (ANDed) with the provided `mask`.
    /// Otherwise, the value is returned as-is.
    pub fn mask(&self, mask: u64) -> Self {
        match self {
            Value::Num(n) => Value::Num(n & mask),
            Value::Bytes(items) => Value::Bytes(items),
        }
    }
